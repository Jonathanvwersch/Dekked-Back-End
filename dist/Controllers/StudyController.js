"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.StudyController = void 0;
const ClassicCardModel_1 = __importDefault(require("../Persistance/ClassicCardModel"));
const StudyService_1 = __importDefault(require("../Services/StudyService"));
class StudyController {
    getSpacedRepetitionCards(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const response = yield ClassicCardModel_1.default.getAllCards();
                const cards = StudyService_1.default.getSpacedRepetitionQueuedCards(response);
                return res.status(200).json({ success: true, data: cards });
            }
            catch (e) {
                console.log(e.message);
                return res.status(500).json({ success: false, error: e.message });
            }
        });
    }
}
exports.StudyController = StudyController;
